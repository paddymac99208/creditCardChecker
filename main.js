...
const validateCred = (arr) => {
  let numberCheck = arr.map((x) => x);
  //console.log(numberCheck);
  const checkDigit = numberCheck.pop();
  for (let i = numberCheck.length - 1; i >= 0; i -= 2) {
    if (numberCheck[i] * 2 > 9) {
      numberCheck[i] = numberCheck[i] * 2 - 9;
    } else {
      numberCheck[i] = numberCheck[i] * 2;
    }
  }
  numberCheck.push(checkDigit);
  //Sum the array
  let initialValue = 0;
  const sumOfArray = numberCheck.reduce(
    (previousValue, currentValue) => previousValue + currentValue,
    initialValue
  );
  let modulus = sumOfArray % 10;
  //console.log(numberCheck);
  //console.log(arr);
...
const idInvalidCardCompanies = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i][0] === 3) {
      ...
      if (cardCompanies.indexOf("Master Card") === -1) {
